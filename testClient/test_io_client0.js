...
var device_info = {device_name : 'TestDevice0', data : [{time : 0},
        {data1 :{x: 0 ,y:0}}] };
...
io.on('start to send',function(condition){
  ...
  var dataSending = setInterval(function(){
    pulse_obj.data[1].data1.x = Math.sin(2*pi*pulse_obj.data[0].time*v);
    pulse_obj.data[1].data1.y = Math.cos(2*pi*pulse_obj.data[0].time*2*v);
